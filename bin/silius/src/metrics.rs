...
use serde::Serialize;
use silius_grpc::metrics::describe_grpc_metrics;
use silius_mempool::metrics::describe_mempool_metrics;
use silius_rpc::metrics::describe_json_rpc_metrics;
    ...
    E: Send + Sync + Debug,
{
    match result {
        Ok(res) => {
            counter!(format!("{request_type}.success")).increment(1);
            Ok(res)
        }
        Err(e) => {
            counter!(format!("{request_type}.failed")).increment(1);
...
pub fn launch_metrics_exporter(listen_addr: SocketAddr, label_value_opt: Option<LabelValue>) {
    let mut builder = PrometheusBuilder::new();
    info!("launching Prometheus metrics exporter on {}", listen_addr);
    if let Some(LabelValue { label, value }) = label_value_opt {
